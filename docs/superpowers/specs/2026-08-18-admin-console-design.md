...
- 日期：2026-08-18
- 状态：已实现并加固（2026-08-18）。P0/P1/P2 主体 + 安全加固（真实脱敏摘要、审计同事务、可信代理 IP 门控、生产 Secure Cookie 强制、禁用管理员禁止续期、旧静态邀请端点生产禁用、敏感授权跨度上限与单事务消费）+ 管理员生命周期收口（列表/停用/启用/自助改密，停用即撤会话、自锁保护）+ 总览扩展指标与用量趋势 + 审计筛选与时间范围上限 + API 级与浏览器级 e2e（e2e/admin.spec.ts / e2e/admin-ui.spec.ts）
