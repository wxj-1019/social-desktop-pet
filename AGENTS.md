...
| 改在线状态/收件箱投递/心跳/Presence 广播            | `apps/server/src/realtime/ws.ts`（连接/投递/isOnline）+ `realtime/presence.ts`（presence.changed B 类广播 + last_seen_at 刷新）                                                                                             | 9.2 / 9.4                |
| 改可观测性/HTTP 防护（日志/request-id/安全头/限流） | `apps/server/src/lib/logger.ts`（JSON 日志 + request-id 中间件）+ `lib/http-guard.ts`（body 上限）+ `src/index.ts`（timeout/secureHeaders 挂载）                                                                            | P0-3 / P1-4              |
| 改指标暴露（/metrics Prometheus）                   | `apps/server/src/lib/metrics.ts`（计数器/渲染；METRICS_TOKEN 可选保护）                                                                                                                                                     | P2 观测收尾              |
| 改羁绊（bonds 推进/阶段阈值/pets lazy init）        | `apps/server/src/lib/relationships.ts`（findOrCreatePet/findOrCreateBond/advanceBond）+ `business-rules.ts`（bondStageFor）；送礼/拜访事务内推进，/friends 返回 bond                                                        | 7.4                      |
| 容器化（镜像/compose）                              | `apps/server/Dockerfile`（构建上下文=仓库根）+ 根 `docker-compose.yml`（pgvector + server）；CI docker-build job 验证                                                                                                       | P2 交付                  |
| 改多实例广播（跨实例 WS 投递/presence 聚合）        | `apps/server/src/realtime/pubsub.ts`（PG LISTEN/NOTIFY，单通道+instanceId 去重）+ `ws.ts`（attachPubSub/deliverLocal/远端在线集合 TTL）                                                                                     | 多实例支持               |
| 改桌宠档案跨设备同步                                | `apps/server/src/routes/pet-profile.ts`（GET/PUT /pet/profile，pets.profile_sync）+ `apps/desktop/src/app/app.tsx`（登录拉取/变更上报）+ migration 0018                                                                     | P2 收尾                  |
...
| 新增/适配桌宠形象（manifest/命中区/动作覆盖/许可）  | `apps/desktop/src/pet/character-manifests.ts` + `character-registry.ts` + `character-preflight.ts`（预检 `pnpm preflight:characters`；协议见 `docs/superpowers/specs/2026-08-20-desktop-pet-character-protocol-design.md`） | 桌宠形象协议             |
